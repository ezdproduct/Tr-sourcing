...
function parseUniverWorkbook(data: any): Record<string, string>[] {
  ...
  for (const sheetId of Object.keys(data.sheets)) {
    ...
    const rowKeys = Object.keys(cellData).sort((a, b) => parseInt(a) - parseInt(b))
    ...
    for (const rKey of rowKeys) {
      ...
      for (const cKey of Object.keys(row)) {
        ...
        const val = cell && cell.v !== undefined && cell.v !== null ? String(cell.v).trim().toLowerCase() : ''
        if (val) {
          tempMap[cKey] = val
          // Identify as header if it contains key columns like 'công ty', 'tên', or 'mst'
          if (val.includes('công ty') || val === 'tên' || val === 'mst' || val.includes('company') || val.includes('name')) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isUniverWorkbook) {
      ...
      rawSuppliersToInsert = parsedRows.map((rowRecord) => {
        const companyName = getField(rowRecord, ['company', 'company_name', 'name', 'Công ty', 'công ty', 'Company'])
        const contactPerson = getField(rowRecord, ['contact_person', 'contact', 'primary_contact_name', 'Tên', 'tên', 'Contact Person'])
        const finalName = companyName || contactPerson || 'Unnamed Supplier'
        
        return {
          raw_name: finalName,
          raw_email: getField(rowRecord, ['email', 'Email']),
          raw_phone: getField(rowRecord, ['phone', 'sdt', 'Sdt', 'Sđt', 'Số điện thoại', 'Phone']),
          raw_address: getField(rowRecord, ['address', 'Địa chỉ', 'địa chỉ', 'Address']),
          raw_website: getField(rowRecord, ['website', 'Website']),
          raw_contact_person: contactPerson,
          raw_tax_id: getField(rowRecord, ['tax_id', 'mst', 'MST', 'Mã số thuế']),
          raw_established_date: getField(rowRecord, ['established_date', 'founded_date', 'Ngày hoạt động công ty', 'ngày hoạt động', 'Established Date']),
          raw_payment_terms: getField(rowRecord, ['payment_terms', 'Điều kiện thanh toán', 'điều kiện thanh toán', 'Payment Terms']),
          raw_factory_area: getField(rowRecord, ['factory_area', 'Diện tích nhà xưởng', 'diện tích', 'Factory Area']),
          raw_total_staff: getField(rowRecord, ['total_staff', 'company_size', 'Tổng lực lượng con người', 'nhân sự', 'Total Staff']),
          raw_workers: getField(rowRecord, ['workers', 'Nhân công lao động', 'nhân công', 'Workers']),
          raw_capacity: getField(rowRecord, ['capacity', 'production_capacity', 'Năng lực sản xuất/ tháng', 'năng lực sản xuất', 'Capacity']),
          raw_main_product: getField(rowRecord, ['main_product', 'Chủ lực dòng hàng', 'dòng hàng', 'Main Product']),
          raw_main_wood: getField(rowRecord, ['main_wood', 'Gỗ chủ lực làm', 'gỗ chủ lực', 'Wood']),
          raw_notes: getField(rowRecord, ['notes', 'Ghi chú', 'ghi chú', 'Notes']),
          status: 'pending'
        }
      }).filter((r) => r.raw_name !== 'Unnamed Supplier')
    } else {
      // Flat payload format
      const companyName = getField(record, ['company', 'company_name', 'name', 'Công ty', 'công ty', 'Company'])
      const contactPerson = getField(record, ['contact_person', 'contact', 'primary_contact_name', 'Tên', 'tên', 'Contact Person'])
      ...
      if (finalName) {
        rawSuppliersToInsert.push({
          ...
          raw_phone: getField(record, ['phone', 'sdt', 'Sdt', 'Sđt', 'Số điện thoại', 'Phone']),
          raw_address: getField(record, ['address', 'Địa chỉ', 'địa chỉ', 'Address']),
          raw_website: getField(record, ['website', 'Website']),
          raw_contact_person: contactPerson,
          raw_tax_id: getField(record, ['tax_id', 'mst', 'MST', 'Mã số thuế']),
          raw_established_date: getField(record, ['established_date', 'founded_date', 'Ngày hoạt động công ty', 'ngày hoạt động', 'Established Date']),
          raw_payment_terms: getField(record, ['payment_terms', 'Điều kiện thanh toán', 'điều kiện thanh toán', 'Payment Terms']),
          raw_factory_area: getField(record, ['factory_area', 'Diện tích nhà xưởng', 'diện tích', 'Factory Area']),
          raw_total_staff: getField(record, ['total_staff', 'company_size', 'Tổng lực lượng con người', 'nhân sự', 'Total Staff']),
          raw_workers: getField(record, ['workers', 'Nhân công lao động', 'nhân công', 'Workers']),
          raw_capacity: getField(record, ['capacity', 'production_capacity', 'Năng lực sản xuất/ tháng', 'năng lực sản xuất', 'Capacity']),
          raw_main_product: getField(record, ['main_product', 'Chủ lực dòng hàng', 'dòng hàng', 'Main Product']),
          raw_main_wood: getField(record, ['main_wood', 'Gỗ chủ lực làm', 'gỗ chủ lực', 'Wood']),
